...
```sh
./tests/run.sh            # 484 cases; exit 0 only if all pass
...
| `suite_search.sh` | 34 | `Ctrl+F`, exactly when the highlight and hints appear and go, and the Esc chain. Headless **and** pty: the Esc chain cannot be seen headlessly, because feedkeys force-ends Insert as the typeahead drains |
